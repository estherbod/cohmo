...
const TableStatusName = ['Calling', 'Coordination', 'Busy', 'Vacant'];
    ...
    computed: {
        start_time: function() {
            ...
            for (let table of this.tables) {
                ...
                else if (table.status == TableStatus.BUSY || table.status == TableStatus.VACANT) curr = this.now + 300;
...
Vue.component('queue-header', {
    ...
    computed: {
        ...
        vacant: function() {
            return this.table.status == TableStatus.VACANT;
        },
...
<div v-bind:class='"queue-header " + [[status_name]]'>
    <div class='queue-header-container'>
        ...
        <div class='vacant' v-if='vacant'> vacant </div>
    </div>
</div>`,
});
